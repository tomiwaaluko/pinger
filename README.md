...
Personal GitHub Actions watcher. It polls a fleet of Greenhouse job boards a few times a day and pings **one** Discord channel when a new intern, co-op, or new-grad Software Engineer or AI Engineer role appears at any enabled company.
...
1. Loads `companies.yaml` and fetches only companies with `enabled: true` (concurrency 10, 20s request timeout, `content=true` for job descriptions).
...
5. On later new hits, applies a fair round-robin soft cap (≤25 jobs per run before LLM/Discord), reads `Career/` from a private Obsidian vault repo, asks Gemini for a short fit note, and posts a Discord embed whose URL is the Greenhouse `absolute_url`. Embed **Company** is the config `name`.

Partial failures are isolated: one bad Greenhouse board does not block other companies. A mid-fleet Discord failure still records successful posts and exits 2.
